        ...
        <Table className="dark:bg-stone-950">
            ...
            <TableBody>
                {data?.map((departure, i) => {
                    const hasCancelledDepartures = departure.Deviations?.some(deviation => deviation?.Consequence === "CANCELLED")
                    return <React.Fragment key={`departure-${departure?.StopAreaName}-${i}`}>
                        <TableRow className={`${departure?.Deviations?.length > 0 && 'border-b-0'} ${hasCancelledDepartures && 'bg-red-400 dark:bg-red-700'}`}>
                        ...
                        {departure?.Deviations?.map((deviation, j) => (
                            <TableRow key={`${deviation.Consequence}-${j}`} className={`border-t-0 ${hasCancelledDepartures && 'dark:bg-red-300 bg-red-200'}`}>
                                    ...
                                    <Accordion type="single" collapsible className="w-full">
                                        <AccordionItem className="my-1 border-b-0" value={`${deviation.Consequence}-j`}>
                                            <AccordionTrigger className="font-medium py-1">
                                                <div>
                                                    <FontAwesomeIcon className={`mr-2 my-auto text-red-700`} icon={hasCancelledDepartures ? faWarning : faInfoCircle} />
